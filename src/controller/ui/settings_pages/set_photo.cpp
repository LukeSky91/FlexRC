...
PhotoItem nextItemOnPage(PhotoItem item, PhotoPage p)
{
    if (p == PhotoPage::Main)
    {
        switch (item)
        {
        case PhotoItem::MinAdc: return PhotoItem::MaxAdc;
        case PhotoItem::MaxAdc: return PhotoItem::MinLed;
        case PhotoItem::MinLed: return PhotoItem::MaxLed;
        case PhotoItem::MaxLed: return PhotoItem::Source;
        case PhotoItem::Source: return PhotoItem::Fixed;
        default: return PhotoItem::MinAdc;
        }
    }

    switch (item)
    {
    case PhotoItem::Filter: return PhotoItem::Hysteresis;
    default: return PhotoItem::Filter;
    }
}
...
void render(bool forceRedraw)
{
    ...
    if (page == PhotoPage::Main)
    {
        ...
        snprintf(line0, sizeof(line0), " ADC %c%4d %c%4d", aMin, currentCfg.minRaw, aMax, currentCfg.maxRaw);
        snprintf(line1, sizeof(line1), " LED %c%3u%% %c%3u%%", bMin, (unsigned)currentCfg.minLedPct, bMax, (unsigned)currentCfg.maxLedPct);
        snprintf(line2, sizeof(line2), "%cSRC  %s", src, modeLabel(currentCfg.mode));
        snprintf(line3, sizeof(line3), "%cFIX %3u%%", fix, (unsigned)currentCfg.fixedPct);
    }
    else
    {
    ...
    char footerLeft[16];
    snprintf(footerLeft, sizeof(footerLeft), "R%4d %3u%%",
             photoSensorReadRaw(),
             (unsigned)photoSensorReadMappedPct());
    snprintf(footer, sizeof(footer), "%-15.15s[%u/2]",
             footerLeft,
             (unsigned)page + 1);
